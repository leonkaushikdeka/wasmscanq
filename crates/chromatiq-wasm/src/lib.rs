...
use chromatiq_core::{BamReader, Coverage, Pileup, PileupConfig, RegionStats};
...
impl BamFile {
    ...
    pub fn new(data: &[u8]) -> Result<BamFile, JsValue> {
        ...
        let mut reader = BamReader::new(&temp_path)
    ...
    pub fn calculate_coverage(
        ...
        let reader = self
            .reader
            .as_mut()
            .ok_or_else(|| JsValue::from_str("No BAM loaded"))?;
        let ref_id = reader
            .reference_names()
            .iter()
            .position(|n| n == &ref_name)
            .ok_or_else(|| JsValue::from_str("Ref not found"))? as i32;
        let coverage = chromatiq_core::calculate_coverage_simple(reader, ref_id, start, end)
            .map_err(|e| JsValue::from_str(&format!("Coverage error: {:?}", e)))?;
        serde_wasm_bindgen::to_value(&CoverageData::from(coverage))
            .map_err(|e| JsValue::from_str(&format!("Ser error: {:?}", e)))
    }
    ...
    pub fn generate_pileup(
        ...
        let ref_id = reader
            ...
            .position(|n| n == &ref_name)
            .ok_or_else(|| JsValue::from_str("Ref not found"))? as i32;
        let mut config = PileupConfig {
            min_mapq,
            min_baseq,
            ..Default::default()
        };
        let pileup = chromatiq_core::generate_pileup(reader, ref_id, start, end, Some(config))
            .map_err(|e| JsValue::from_str(&format!("Pileup error: {:?}", e)))?;
        serde_wasm_bindgen::to_value(&PileupData::from(pileup))
            .map_err(|e| JsValue::from_str(&format!("Ser error: {:?}", e)))
    }
    ...
    pub fn get_region_stats(
        ...
        serde_wasm_bindgen::to_value(&RegionStatsData {
        ...
        .map_err(|e| JsValue::from_str("Ser error"))
    }
}
...
impl VcfFile {
    ...
    pub fn new(data: &[u8]) -> Result<VcfFile, JsValue> {
        ...
        let reader = chromatiq_core::VcfReader::new(&content);
        let stats = chromatiq_core::VcfStats::from_reader(&reader);
...
impl ChromatiqEngine {
    ...
    pub fn unload_bam(&mut self, index: usize) -> Result<(), JsValue> {
        if index >= self.bam_files.len() {
            return Err(JsValue::from_str("Invalid BAM index"));
        }
    ...
    pub fn unload_vcf(&mut self, index: usize) -> Result<(), JsValue> {
        if index >= self.vcf_files.len() {
            return Err(JsValue::from_str("Invalid VCF index"));
        }
    ...
    pub fn calculate_combined_coverage(
        ...
        for bam in &mut self.bam_files {
            ...
            let cov_data: CoverageData = serde_wasm_bindgen::from_value(coverage)
                .map_err(|e| JsValue::from_str(&format!("Deser error: {:?}", e)))?;
        ...
        serde_wasm_bindgen::to_value(&CombinedCoverageData {
    ...
    pub fn get_all_variants(&self) -> Result<JsValue, JsValue> {
        let all_variants: Vec<Vec<VariantData>> = self
            .vcf_files
            .iter()
            .enumerate()
            .map(|(file_idx, vcf)| {
                let mut file_variants: Vec<VariantData> = vcf.variants.clone();
...
pub fn get_info() -> String {
    format!(
        "Chromatiq {} - WASM Genomic Visualization",
...
pub fn get_capabilities() -> String {
    r#"{"bam":true,"vcf":true,"cram":false}"#.to_string()
}
